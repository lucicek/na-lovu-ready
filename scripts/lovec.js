...
// 🔹 Unikátní session ID pro lovec
const sessionId = '234567';
...
// Stisk X na PC → červeně označí volbu
document.addEventListener('keydown', e => {
  if(e.key.toLowerCase() === 'x'){
    // vyčistit značení všech tlačítek
    optionButtons.forEach(b => { b.style.backgroundColor = ''; b.style.color = ''; b.style.borderColor = '#333'; });

    // pokud už máme currentSelection, použijeme ho
    if(currentSelection){
      const btn = document.querySelector(`.option[data-option="${currentSelection}"]`);
      if(btn){ btn.style.borderColor = 'red'; btn.style.borderWidth = '3px'; }
      return;
    }
    ...
    (async () => {
      ...
      if(sel){
        ...
        if(btn){ btn.style.borderColor = 'red'; btn.style.borderWidth = '3px'; currentSelection = sel; }
      }
    })();
  }
});